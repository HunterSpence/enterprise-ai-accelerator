...
SEVERITY_DEFINITIONS: dict[str, dict] = {
    IncidentSeverity.P0: {
        ...
        "eu_ai_act_article": "Article 73 — Serious incident reporting",
...
RESPONSE_PLAYBOOKS: dict[str, list[dict]] = {
    IncidentSeverity.P0: [
        ...
        {
            "step": 5,
            "action": "Draft Article 73 serious incident notification",
            "owner": "Legal + Compliance",
            "timeframe": "T+24h",
            "details": "EU AI Act Article 73 requires reporting of serious incidents under tiered deadlines (not a universal 72-hour rule); verify the applicable period against the current official text. Not legal advice. Use PolicyGuard Article 73 template.",
            "automated": False,
        },
        {
            "step": 6,
...
# ---------------------------------------------------------------------------
# Article 73 notification template
# ---------------------------------------------------------------------------

def generate_article_73_notification(
    incident_id: str,
    system_name: str,
    ...
    """Generate an EU AI Act Article 73 serious-incident notification template.

    Template only — not legal advice. Article 73 deadlines are tiered, not a
    universal 72 hours; verify against the current official text.
    """
...
Article 73, Regulation (EU) 2024/1689
==========================================

NOTIFICATION REF: {incident_id}
DATE OF NOTIFICATION: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}
...
Severity Classification: P0 — Safety-Critical / Serious Incident (Article 73)
...
T+(per Art. 73 tiered deadline): Serious-incident report filed with supervisory authority
...
Generated by PolicyGuard v2.0 | EU AI Act Article 73 Compliance Module
...
class IncidentResponse:
    ...
    article_73_notification: Optional[str]
...
class IncidentClassifier:
    ...
    def generate_response(
        ...
        # Article 73 notification required for P0
        article_73 = None
        if incident.severity == IncidentSeverity.P0:
            article_73 = generate_article_73_notification(
        ...
        return IncidentResponse(
            ...
            article_73_required=incident.severity == IncidentSeverity.P0,
            article_73_notification=article_73,
    ...
    def run_demo(self) -> IncidentReport:
        ...
        for i, demo in enumerate(DEMO_INCIDENTS):
            ...
            incident = AIIncident(
                ...
                system_name=demo["system_name"],
                detected_at=datetime.now(timezone.utc) - timedelta(hours=i * 3),
                severity=severity,
                title=demo["title"],
                description=demo["description"],
        ...
        return IncidentReport(
            ...
            regulatory_notifications_required=sum(
                1 for r in responses if r.article_73_required
            ),
        )
